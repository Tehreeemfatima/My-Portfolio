  ...
  return (
    ...
    <ul className=' hidden md:flex'>
         {/* Method No 1 */}
          ...
          {/* Method No 2 */}
          {Links.map(({id,link})=>(
             <li key={id} className='px-4 cursor-pointer capitalize font-medium text-gray-500 hover:scale-105 duration-200 hover:text-cyan-500 '><Link to={link} smooth duration={500}>{link}</Link></li>
          ))}     
     </ul>
     {/* Mobile icons */}
     ...
     {nav && ( 
      ...
      from-black to-gray-800">
         {Links.map(({id,link})=>(
             <li key={id} className='px-4 cursor-pointer capitalize py-6 text-4xl hover:text-cyan-500'><Link onClick={()=>setNav(!nav)} to={link} smooth duration={500}>{link}</Link></li> ))}   
      </ul> )}
